...
namespace E_Procurement.Repository.DINRepo
{
    public class GRNRepository : IGRNRepository
    {
        ...
        public GRNRepository(EProcurementContext context, IConvertViewToPDF pdfConverter, IPORepository poRepository, IHttpContextAccessor contextAccessor)
        {
            _context = context;
        ...
        public async  Task<bool> GRNGenerationAsync(RFQGenerationModel rfq)
        {
            if (rfq != null)
            {
                var poNumber = await _context.PoGenerations.Where(x => x.PONumber == rfq.PONumber).FirstOrDefaultAsync();
                GRNGeneration dnDetails = new GRNGeneration
                {
                    
                    POId = poNumber.Id,
                    GRNUploadedDate = DateTime.Now,
                    GRNFilePath= rfq.GRNFilePath,
                    GRNUploadedBy= _contextAccessor.HttpContext.User.Identity.Name,
                };
        ...
        public async Task<IEnumerable<RFQGenerationModel>> GetPOAsync()
        {
            var query = await (from po in _context.PoGenerations
                               ...
                               where !(from gn in _context.GrnGenerations select gn.POId).Contains(po.Id)
